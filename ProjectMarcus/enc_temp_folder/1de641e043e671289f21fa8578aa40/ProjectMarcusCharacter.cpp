...
void AProjectMarcusCharacter::MoveForward(float Value)
{
	if (Controller && Value != 0.f)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(0, -1.f, FColor::Blue, FString::Printf(TEXT("Walk speed: %f"), GetVelocity().Size()));
		}
	}
}
...
void AProjectMarcusCharacter::CheckForItemsInRange()
{
	if (ItemsInRange.Num())
	{
		...
		DrawDebugLine(GetWorld(), CrosshairLocationInWorld, LookDir * 50'000, FColor::Blue);
